...
half4 applyHSV(half4 rgba, half4 hsva, half minRange, half maxRange) {
	...
	// Do we do any masking on the shift?
		...
		// Apply hue and saturation to the selected range only
		half3 hsvShift = (- 1.0 + hsva.rgb * 2.0);
		rgbShifted.rg += hsvShift.rg * affectMult; // Apply H&S based on range
		rgbShifted = hsv2rgb(rgbShifted);
		
		// Apply brightness to the entire image regardless of hue range
		rgbShifted = rgbShifted * (1.0 + hsvShift.b);

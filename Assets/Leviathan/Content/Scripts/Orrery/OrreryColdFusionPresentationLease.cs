...
public static class OrreryColdFusionPresentationLease
{
    ...
    public static void ObserveGrant(NetSession session, CoreCrossOwnerEffects.GrantNotice notice)
    {
        float duration = new FloatBits { UInt = notice.Payload.A }.Float;
        Remember(session, notice.SourcePlayerId, notice.TargetPlayerId, notice.Sequence, duration);
    }

...
public static class OrrerySpellRuntime
{
        ...
        OrrerySpellRegistry.SpellDefinition spell)
    {
        ...
        if (cryo == null || !cryo.CanActivate() || !OrreryDamageRouter.EnsureAvailable())
            return false;
        ...
        Vector2 hitPoint)
    {
        ...
        foreach (KeyValuePair<GameShip, OwnerState> pair in owners)
        {
            ...
            if (launcher == null || launcher.ExplosiveRadius <= 0f ||
                !OrreryDamageRouter.EnsureAvailable())
            {
                return;
            }
        ...
        Activatable slotSource)
    {
        ...
        return OrreryDamageRouter.Route(
            fromShip, damageable, damageType, damageData, statusEffectChance,
            crit, fromPosition, bypassDamageLimit, knockbackPower, slotSource);
    }
}
